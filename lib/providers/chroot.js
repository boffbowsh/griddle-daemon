...
function Process(slugId, command, env, appName) {
  ...
  this.spawn = function (){
    ...
    self.childProcess.on("exit", function() {
      console.log("exit:", self, self.exiting);
      if (!self.exiting) {
        console.log("respawn");
        self.spawn();
      }
    ...
    self.childProcess.stdout.on("data", function(data) { self.emit("stdout", data); });
    self.childProcess.stderr.on("data", function(data) { self.emit("stderr", data); });
  };
